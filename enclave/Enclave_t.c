...
static sgx_status_t SGX_CDECL sgx_create_sealeddata_for_serializable(void* pms)
{
	...
	ms_create_sealeddata_for_serializable_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_create_sealeddata_for_serializable_t), ms, sizeof(ms_create_sealeddata_for_serializable_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_sealed_log = __in_ms.ms_sealed_log;
	uint32_t _tmp_sealed_log_size = __in_ms.ms_sealed_log_size;
	...
	_in_retval = create_sealeddata_for_serializable(_in_sealed_log, _tmp_sealed_log_size);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_sealed_log) {
		if (memcpy_verw_s(_tmp_sealed_log, _len_sealed_log, _in_sealed_log, _len_sealed_log)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
static sgx_status_t SGX_CDECL sgx_verify_sealeddata_for_serializable(void* pms)
{
	...
	ms_verify_sealeddata_for_serializable_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_verify_sealeddata_for_serializable_t), ms, sizeof(ms_verify_sealeddata_for_serializable_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_sealed_log = __in_ms.ms_sealed_log;
	...
	_in_retval = verify_sealeddata_for_serializable(_in_sealed_log, _tmp_sealed_log_size);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_create_sealeddata_for_fixed(void* pms)
{
	...
	ms_create_sealeddata_for_fixed_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_create_sealeddata_for_fixed_t), ms, sizeof(ms_create_sealeddata_for_fixed_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_sealed_log = __in_ms.ms_sealed_log;
	...
	_in_retval = create_sealeddata_for_fixed(_in_sealed_log, _tmp_sealed_log_size);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_verify_sealeddata_for_fixed(void* pms)
{
	...
	ms_verify_sealeddata_for_fixed_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_verify_sealeddata_for_fixed_t), ms, sizeof(ms_verify_sealeddata_for_fixed_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_sealed_log = __in_ms.ms_sealed_log;
	...
	_in_retval = verify_sealeddata_for_fixed(_in_sealed_log, _tmp_sealed_log_size);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
